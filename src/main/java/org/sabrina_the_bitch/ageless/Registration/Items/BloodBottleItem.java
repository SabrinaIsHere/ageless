    ...
    public ItemStack finishUsing(ItemStack stack, World world, LivingEntity user) {
        ...
        if (user instanceof PlayerEntity) {
            ...
            if (((PlayerVampirism) player).isVampire().get()) {
                player.getHungerManager().add(6, 2.0F);
                player.addStatusEffect(new StatusEffectInstance(StatusEffects.REGENERATION, 3 * 20));
                player.addStatusEffect(new StatusEffectInstance(StatusEffects.STRENGTH, 7 * 20));
            } else {
